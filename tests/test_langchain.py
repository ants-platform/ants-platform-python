...
def test_callback_generated_from_trace_chain():
    ...
    with ants_platform.start_as_current_span(name="parent") as span:
...
def test_callback_simple_openai():
    ...
    with ants_platform.start_as_current_span(name="simple_openai_test") as span:
        trace_id = span.trace_id
...
def test_callback_multiple_invocations_on_different_traces():
    ...
    with ants_platform.start_as_current_span(name="multiple_invocations_test") as span:
        trace_id = span.trace_id
...
def test_callback_openai_functions_python():
    ...
    with ants_platform.start_as_current_span(name="openai_functions_python_test") as span:
        trace_id = span.trace_id
        handler = CallbackHandler()
...
def test_agent_executor_chain():
    ...
    with ants_platform.start_as_current_span(name="agent_executor_chain_test") as span:
        trace_id = span.trace_id
...
def test_unimplemented_model():
    ...
    with ants_platform.start_as_current_span(name="unimplemented_model_test") as span:
        trace_id = span.trace_id
        callback = CallbackHandler()
...
def test_openai_instruct_usage():
    ...
    with ants_platform.start_as_current_span(name="openai_instruct_usage_test") as span:
        trace_id = span.trace_id
...
def test_get_langchain_prompt_with_jinja2():
    ...
    ants_platform_prompt = ants_platform.get_prompt(
        "test_jinja2", fetch_timeout_seconds=1, max_retries=3
    )

    assert (
        ants_platform_prompt.get_langchain_prompt()
...
def test_get_langchain_prompt():
    ...
    for i, test_prompt in enumerate(test_prompts):
        ...
        ants_platform_prompt = ants_platform.get_prompt(f"test_{i}")
...
def test_get_langchain_chat_prompt():
    ...
    for i, test_prompt in enumerate(test_prompts):
        ...
        ants_platform_prompt = ants_platform.get_prompt(f"test_chat_{i}", type="chat")
...
def test_link_ants_platform_prompts_invoke():
    ants_platform = AntsPlatform()
    trace_name = "test_link_ants_platform_prompts_invoke"
    ...
    # Get prompts
    ants_platform_joke_prompt = ants_platform.get_prompt(joke_prompt_name)
    ants_platform_explain_prompt = ants_platform.get_prompt(explain_prompt_name)

    langchain_joke_prompt = PromptTemplate.from_template(
        ants_platform_joke_prompt.get_langchain_prompt(),
        metadata={"ants_platform_prompt": ants_platform_joke_prompt},
    )

    langchain_explain_prompt = PromptTemplate.from_template(
        ants_platform_explain_prompt.get_langchain_prompt(),
        metadata={"ants_platform_prompt": ants_platform_explain_prompt},
    )
    ...
    # Run chain
    ants_platform_handler = CallbackHandler()

    with ants_platform.start_as_current_span(name=trace_name) as span:
        trace_id = span.trace_id
        chain.invoke(
            {"animal": "dog"},
            config={
                "callbacks": [ants_platform_handler],
                "run_name": trace_name,
            },
        )

    ants_platform_handler.client.flush()
    sleep(2)
    ...
    assert generations[0].prompt_version == ants_platform_joke_prompt.version
    assert generations[1].prompt_version == ants_platform_explain_prompt.version


def test_link_ants_platform_prompts_stream():
    ants_platform = AntsPlatform()
    trace_name = "test_link_ants_platform_prompts_stream"
...
def test_link_ants_platform_prompts_batch():
    ants_platform = AntsPlatform()
    trace_name = "test_link_ants_platform_prompts_batch_" + create_uuid()[:8]
    ...
    assert generations[0].prompt_version == ants_platform_joke_prompt.version
    assert generations[1].prompt_version == ants_platform_joke_prompt.version
    assert generations[2].prompt_version == ants_platform_joke_prompt.version
    assert generations[3].prompt_version == ants_platform_explain_prompt.version
    assert generations[4].prompt_version == ants_platform_explain_prompt.version
    assert generations[5].prompt_version == ants_platform_explain_prompt.version
...
def test_get_langchain_text_prompt_with_precompiled_prompt():
    ...
    ants_platform_prompt = ants_platform.get_prompt(prompt_name)
    langchain_prompt = PromptTemplate.from_template(
        ants_platform_prompt.get_langchain_prompt(pre_compiled_var="dog")
    )
...
def test_get_langchain_chat_prompt_with_precompiled_prompt():
    ...
    ants_platform_prompt = ants_platform.get_prompt(prompt_name, type="chat")
    langchain_prompt = ChatPromptTemplate.from_messages(
        ants_platform_prompt.get_langchain_prompt(pre_compiled_var="dog")
    )
...
def test_ants_platform_overhead():
    ...
    # Test performance overhead of ants_platform tracing
    inputs = _generate_random_dict(10000, 20000)
    test_chain = RunnableLambda(lambda x: None)

    start = time.monotonic()
    test_chain.invoke(inputs)
    duration_without_ants_platform = (time.monotonic() - start) * 1000

    start = time.monotonic()
    handler = CallbackHandler()
    ants_platform = AntsPlatform()

    with ants_platform.start_as_current_span(name="test_ants_platform_overhead"):
        test_chain.invoke(inputs, config={"callbacks": [handler]})

    duration_with_ants_platform = (time.monotonic() - start) * 1000

    overhead = duration_with_ants_platform - duration_without_ants_platform
    print(f"AntsPlatform overhead: {overhead}ms")

    assert (
        overhead < 100
    ), f"AntsPlatform tracing overhead of {overhead}ms exceeds threshold"
...
def test_multimodal():
    ...
    assert (
        "@@@ants_platformMedia:type=image/jpeg|id="
...
def test_langchain_automatic_observation_types():
    ...
    with ants_platform.start_as_current_span(name="observation_types_test_agent") as span:
        trace_id = span.trace_id
        handler = CallbackHandler()
