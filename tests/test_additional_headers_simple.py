"""Simplified tests for additional_headers functionality in AntsPlatform client.
...
class TestAdditionalHeadersSimple:
    ...
    def test_httpx_client_has_additional_headers_when_none_provided(self):
        ...
        assert (
            ants_platform._resources.httpx_client.headers["X-Custom-Header"]
            == "custom-value"
        )
        assert (
            ants_platform._resources.httpx_client.headers["X-Another-Header"]
            == "another-value"
        )
    ...
    def test_custom_httpx_client_with_additional_headers_ignores_additional_headers(
        ...
        assert ants_platform._resources.httpx_client is custom_client

        # Verify existing headers are preserved and additional headers are NOT added
        assert (
            ants_platform._resources.httpx_client.headers["x-existing-header"]
            == "existing-value"
        )

        # Additional headers should NOT be present
        assert "x-custom-header" not in ants_platform._resources.httpx_client.headers
        assert "x-another-header" not in ants_platform._resources.httpx_client.headers
    ...
    def test_none_additional_headers_works(self):
        ...
        assert ants_platform is not None
        assert ants_platform._resources is not None
        assert ants_platform._resources.httpx_client is not None
    ...
    def test_span_processor_has_additional_headers_in_otel_exporter(self):
        ...
        assert "x_ants_platform_sdk_name" in exporter._headers
        assert "x_ants_platform_public_key" in exporter._headers
