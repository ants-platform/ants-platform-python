...
async def test_concurrency():
    ...
    async def update_generation(i, ants_platform: AntsPlatform):
        # Create a new trace with a generation
        with ants_platform.start_as_current_span(name=f"parent-{i}") as parent_span:
            # Set trace name
            parent_span.update_trace(name=str(i))

            # Create generation as a child
            generation = ants_platform.start_generation(name=str(i))
    ...
    # Create AntsPlatform client
    ants_platform = AntsPlatform()

    # Run concurrent operations
    await gather(*(update_generation(i, ants_platform) for i in range(100)))
...
def test_flush():
    # Initialize AntsPlatform client with debug disabled
    ants_platform = AntsPlatform()

    trace_ids = []
    for i in range(2):
        # Create spans and set the trace name using update_trace
        with ants_platform.start_as_current_span(name="span-" + str(i)) as span:
            span.update_trace(name=str(i))
            # Store the trace ID for later verification
            trace_ids.append(ants_platform.get_current_trace_id())

    # Flush all pending spans to the AntsPlatform API
    ants_platform.flush()
...
def test_invalid_score_data_does_not_raise_exception():
    ...
    with ants_platform.start_as_current_span(name="test-span") as span:
        span.update_trace(
...
def test_score_trace_nested_observation():
    ...
    with ants_platform.start_as_current_span(name="parent-span") as parent_span:
        parent_span.update_trace(name=trace_name)

        # Create a child span
        child_span = ants_platform.start_span(name="span")
...
def test_create_trace_and_span():
    ...
    with ants_platform.start_as_current_span(name=trace_name) as parent_span:
        parent_span.update_trace(name=trace_name)
...
def test_create_generation_and_trace():
    ...
    trace_context = {"trace_id": ants_platform.create_trace_id()}
...
def test_create_span_and_get_observation():
    ...
    # Create span
    span = ants_platform.start_span(name="span")
...
def test_update_generation():
    ...
    # Create a generation
    generation = ants_platform.start_generation(name="generation")
...
def test_get_generations():
    ...
    generation1 = ants_platform.start_generation(
        name=create_uuid(),
    )
    generation1.end()
    ...
    generation2 = ants_platform.start_generation(
...
def test_get_generations_by_user():
    ...
    with ants_platform.start_as_current_span(name="test-user") as parent_span:
    ...
    other_gen = ants_platform.start_generation(name="other-generation")
    other_gen.end()
...
def test_timezone_awareness():
    ...
    with ants_platform.start_as_current_span(name="test") as parent_span:
        # Set the trace name
...
def test_get_trace_by_session_id():
    ...
    with ants_platform.start_as_current_span(name=create_uuid()):
        pass
...
def test_fetch_traces():
    ...
    # First trace
    with ants_platform.start_as_current_span(name="test1") as span:
        span.update_trace(
            name=name,
    ...
    # Second trace
    with ants_platform.start_as_current_span(name="test2") as span:
        span.update_trace(
            name=name,
            session_id="session-1",
            input={"key": "value"},
    ...
    # Third trace
    with ants_platform.start_as_current_span(name="test3") as span:
        span.update_trace(
            name=name,
            session_id="session-1",
            input={"key": "value"},
...
def test_get_sessions():
    ...
    # Create first trace
    with ants_platform.start_as_current_span(name=name) as span1:
        span1.update_trace(name=name, session_id=session1)

    # Create second trace
    with ants_platform.start_as_current_span(name=name) as span2:
        span2.update_trace(name=name, session_id=session2)

    # Create third trace
    with ants_platform.start_as_current_span(name=name) as span3:
...
def test_create_trace_sampling_zero():
    ants_platform = AntsPlatform(sample_rate=0)
    api_wrapper = LangfuseAPI()
    trace_name = create_uuid()
    ...
    with ants_platform.start_as_current_span(name="test-span") as span:
        ...
        ants_platform.score_current_trace(name="score", value=0.5)
...
def test_mask_function():
    ...
    ants_platform = AntsPlatform(mask=mask_func)
    api_wrapper = LangfuseAPI()

    # Create a root span with trace properties
    with ants_platform.start_as_current_span(name="test-span") as root_span:
...
def test_get_project_id():
    ants_platform = AntsPlatform()
    res = ants_platform._get_project_id()
...
def test_generate_trace_id():
    ...
    project_id = ants_platform._get_project_id()
    trace_url = ants_platform.get_trace_url(trace_id=trace_id)
...
def test_start_as_current_observation_types():
    ...
    with ants_platform.start_as_current_span(name="parent") as parent_span:
