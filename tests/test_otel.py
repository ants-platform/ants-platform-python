...
class TestOTelBase:
    ...
    def tracer_provider(self, memory_exporter):
        ...
        resource = Resource.create({"service.name": "ants_platform-test"})
    ...
    def mock_processor_init(self, monkeypatch, memory_exporter):
        """Mock the AntsPlatformSpanProcessor initialization to avoid HTTP traffic."""
    ...
    @pytest.fixture
    def ants_platform_client(self, monkeypatch, tracer_provider, mock_processor_init):
        ...
        client._otel_tracer = tracer_provider.get_tracer("ants_platform-test")

        yield client

    @pytest.fixture
    def configurable_ants_platform_client(
        self, monkeypatch, tracer_provider, mock_processor_init
    ):
        """Create a AntsPlatform client fixture that allows configuration parameters."""
...
class TestBasicSpans(TestOTelBase):
    ...
    def test_basic_span_creation(self, ants_platform_client, memory_exporter):
        """Test that a basic span can be created with attributes."""
        # Create a span and end it
        span = ants_platform_client.start_span(name="test-span", input={"test": "value"})
        span.end()
    ...
    def test_span_hierarchy(self, ants_platform_client, memory_exporter):
        """Test creating nested spans and verify their parent-child relationships."""
        # Create parent span
        with ants_platform_client.start_as_current_span(name="parent-span") as parent_span:
            # Create a child span
    ...
    def test_update_current_span_name(self, ants_platform_client, memory_exporter):
        """Test updating current span name via update_current_span method."""
        # Create a span using context manager
        with ants_platform_client.start_as_current_span(name="original-current-span"):
            # Update the current span name
            ants_platform_client.update_current_span(name="updated-current-span")
    ...
    def test_span_attributes(self, ants_platform_client, memory_exporter):
        """Test that span attributes are correctly set and updated."""
        # Create a span with attributes
        span = ants_platform_client.start_span(
            name="attribute-span",
    ...
    def test_span_name_update(self, ants_platform_client, memory_exporter):
        """Test updating span name via update method."""
        # Create a span with initial name
        span = ants_platform_client.start_span(name="original-span-name")
    ...
    def test_generation_span(self, ants_platform_client, memory_exporter):
        """Test creating a generation span with model-specific attributes."""
        # Create a generation
        generation = ants_platform_client.start_generation(
    ...
    def test_generation_name_update(self, ants_platform_client, memory_exporter):
    ...
    def test_trace_update(self, ants_platform_client, memory_exporter):
        """Test updating trace level attributes."""
        # Create a span and update trace attributes
        with ants_platform_client.start_as_current_span(name="trace-span") as span:
            span.update_trace(
    ...
    def test_complex_scenario(self, ants_platform_client, memory_exporter):
        """Test a more complex scenario with multiple operations and nesting."""
        # Create a trace with a main span
        with ants_platform_client.start_as_current_span(name="main-flow") as main_span:
    ...
    def test_update_current_generation_name(self, ants_platform_client, memory_exporter):
        """Test updating current generation name via update_current_generation method."""
        # Create a generation using context manager
        with ants_platform_client.start_as_current_generation(
            name="original-current-generation", model="gpt-4"
        ):
            # Update the current generation name
            ants_platform_client.update_current_generation(name="updated-current-generation")
    ...
    def test_start_as_current_observation_types(self, ants_platform_client, memory_exporter):
        ...
        for obs_type in observation_types:
            with ants_platform_client.start_as_current_observation(
    ...
    def test_start_observation(self, ants_platform_client, memory_exporter):
        ...
        with ants_platform_client.start_as_current_span(
        ...
        for obs_type, obs_instance in created_observations:
            if obs_type == "span":
                from ants_platform._client.span import AntsPlatformSpan

                assert isinstance(
                    obs_instance, AntsPlatformSpan
                ), f"Expected AntsPlatformSpan, got {type(obs_instance)}"
            elif obs_type == "generation":
                from ants_platform._client.span import AntsPlatformGeneration

                assert isinstance(
                    obs_instance, AntsPlatformGeneration
                ), f"Expected AntsPlatformGeneration, got {type(obs_instance)}"
            elif obs_type == "agent":
                from ants_platform._client.span import AntsPlatformAgent

                assert isinstance(
                    obs_instance, AntsPlatformAgent
                ), f"Expected AntsPlatformAgent, got {type(obs_instance)}"
            elif obs_type == "tool":
                from ants_platform._client.span import AntsPlatformTool

                assert isinstance(
                    obs_instance, AntsPlatformTool
                ), f"Expected AntsPlatformTool, got {type(obs_instance)}"
            elif obs_type == "chain":
                from ants_platform._client.span import AntsPlatformChain

                assert isinstance(
                    obs_instance, AntsPlatformChain
                ), f"Expected AntsPlatformChain, got {type(obs_instance)}"
            elif obs_type == "retriever":
                from ants_platform._client.span import AntsPlatformRetriever

                assert isinstance(
                    obs_instance, AntsPlatformRetriever
                ), f"Expected AntsPlatformRetriever, got {type(obs_instance)}"
            elif obs_type == "evaluator":
                from ants_platform._client.span import AntsPlatformEvaluator

                assert isinstance(
                    obs_instance, AntsPlatformEvaluator
                ), f"Expected AntsPlatformEvaluator, got {type(obs_instance)}"
            elif obs_type == "embedding":
                from ants_platform._client.span import AntsPlatformEmbedding

                assert isinstance(
                    obs_instance, AntsPlatformEmbedding
                ), f"Expected AntsPlatformEmbedding, got {type(obs_instance)}"
            elif obs_type == "guardrail":
                from ants_platform._client.span import AntsPlatformGuardrail

                assert isinstance(
                    obs_instance, AntsPlatformGuardrail
                ), f"Expected AntsPlatformGuardrail, got {type(obs_instance)}"
            elif obs_type == "event":
                from ants_platform._client.span import AntsPlatformEvent

                assert isinstance(
                    obs_instance, AntsPlatformEvent
                ), f"Expected AntsPlatformEvent, got {type(obs_instance)}"

    def test_custom_trace_id(self, ants_platform_client, memory_exporter):
    ...
    def test_custom_parent_span_id(self, ants_platform_client, memory_exporter):
    ...
    def test_multiple_generations_in_trace(self, ants_platform_client, memory_exporter):
        ...
        with ants_platform_client.start_as_current_span(name="multi-gen-flow") as main_span:
            # First generation
    ...
    def test_error_handling(self, ants_platform_client, memory_exporter):
        """Test error handling in span operations."""
        # Create a span that will have an error
        span = ants_platform_client.start_span(name="error-span")
...
class TestAdvancedSpans(TestOTelBase):
    ...
    def test_complex_model_parameters(self, ants_platform_client, memory_exporter):
    ...
    def test_updating_current_generation(self, ants_platform_client, memory_exporter):
    ...
    def test_shutdown_and_flush(self, ants_platform_client, memory_exporter):
        """Test shutdown and flush operations."""
        # Create a span without ending it
        span = ants_platform_client.start_span(name="flush-test-span")

        # Explicitly flush
        ants_platform_client.flush()
        ...
        ants_platform_client.start_span(name="shutdown-test-span")

        # Call shutdown (should flush any pending spans)
        ants_platform_client.shutdown()
    ...
    def test_trace_id_generation(self, ants_platform_client):
        """Test trace ID generation follows expected format."""
        # Generate trace IDs
        trace_id1 = ants_platform_client.create_trace_id()
        trace_id2 = ants_platform_client.create_trace_id()
...
class TestMetadataHandling(TestOTelBase):
    ...
    def test_nested_metadata_updates(self):
        ...
        from ants_platform._client.attributes import _flatten_and_serialize_metadata
...
class TestMultiProjectSetup(TestOTelBase):
    ...
    These tests verify that multiple AntsPlatform clients initialized with different
    ...
    def multi_project_setup(self, monkeypatch):
        """Create two separate AntsPlatform clients with different projects."""
        ...
        ants_platform_project1 = AntsPlatform(
            public_key=project1_key, secret_key="secret1", host="http://test-host"
        )

        ants_platform_project2 = AntsPlatform(
        ...
        setup = {
            ...
            "ants_platform_project1": ants_platform_project1,
            "ants_platform_project2": ants_platform_project2,
    ...
    def test_spans_routed_to_correct_exporters(self, multi_project_setup):
        ...
        span1 = multi_project_setup["ants_platform_project1"].start_span(
            name="trace-project1", metadata={"project": "project1"}
        )
        span1.end()

        span2 = multi_project_setup["ants_platform_project2"].start_span(
    ...
    def test_concurrent_operations_in_multiple_projects(self, multi_project_setup):
        ...
        def create_spans_project1():
            for i in range(5):
                span = multi_project_setup["ants_platform_project1"].start_span(
        ...
        def create_spans_project2():
            for i in range(5):
                span = multi_project_setup["ants_platform_project2"].start_span(
...
class TestInstrumentationScopeFiltering(TestOTelBase):
    ...
    def instrumentation_filtering_setup(self, monkeypatch):
        ...
        # Mock the AntsPlatformSpanProcessor to use our test exporters
        def mock_processor_init(self, **kwargs):
            from opentelemetry.sdk.trace.export import BatchSpanProcessor
        ...
        # We'll add the AntsPlatformSpanProcessor to this provider after it's created
        ...
        def mock_initialize(self, **kwargs):
            ...
            # Now create our custom AntsPlatformSpanProcessor with the actual blocked_instrumentation_scopes
            from ants_platform._client.span_processor import AntsPlatformSpanProcessor

            processor = AntsPlatformSpanProcessor(
            ...
            self._otel_tracer = test_tracer_provider.get_tracer(
                "ants_platform-sdk", "test", attributes={"public_key": self.public_key}
            )
    ...
    def test_blocked_instrumentation_scopes_export_filtering(
        ...
        # Create AntsPlatform client with blocked scopes
        AntsPlatform(
        ...
        # Create ants_platform tracer with proper attributes for project validation
        ants_platform_tracer = tracer_provider.get_tracer(
            "ants_platform-sdk",
        ...
        ants_platform_span = ants_platform_tracer.start_span("ants_platform-span")
        ants_platform_span.end()
        ...
        # AntsPlatform spans should be exported (not blocked)
        assert "ants_platform-span" in exported_span_names
        assert "ants_platform-sdk" in exported_scope_names
    ...
    def test_no_blocked_scopes_allows_all_exports(
        ...
        # Create AntsPlatform client with NO blocked scopes
        AntsPlatform(
            public_key=instrumentation_filtering_setup["test_key"],
            secret_key="test-secret-key",
            host="http://localhost:3000",
    ...
    def test_none_blocked_scopes_allows_all_exports(
        ...
        # Create AntsPlatform client with None blocked scopes (default behavior)
        AntsPlatform(
            public_key=instrumentation_filtering_setup["test_key"],
            secret_key="test-secret-key",
            host="http://localhost:3000",
    ...
    def test_blocking_ants_platform_sdk_scope_export(self, instrumentation_filtering_setup):
        """Test that even AntsPlatform's own spans are blocked if explicitly specified."""
        # Create AntsPlatform client that blocks its own instrumentation scope
        AntsPlatform(
            public_key=instrumentation_filtering_setup["test_key"],
            secret_key="test-secret-key",
            host="http://localhost:3000",
            blocked_instrumentation_scopes=["ants_platform-sdk"],
        )
        ...
        # Check exports - AntsPlatform spans should be blocked, others allowed
        exported_spans = instrumentation_filtering_setup[
            "blocked_exporter"
        ].get_finished_spans()
        exported_span_names = [span.name for span in exported_spans]

        assert "ants_platform-span" not in exported_span_names
...
class TestConcurrencyAndAsync(TestOTelBase):
    ...
    @pytest.mark.asyncio
    async def test_async_span_operations(self, ants_platform_client, memory_exporter):
        """Test async operations with spans."""
        import asyncio

        # Start a main span
        main_span = ants_platform_client.start_span(name="async-main-span")
    ...
    def test_context_propagation_async(self, ants_platform_client, memory_exporter):
        ...
        main_span = ants_platform_client.start_span(
        ...
        def thread2_function():
            ...
            thread2_span = ants_platform_client.start_span(
        ...
        def thread3_function():
            ...
            thread3_span = ants_platform_client.start_span(
                name="thread3-span",
    ...
    async def test_span_metadata_updates_in_async_context(
        self, ants_platform_client, memory_exporter
    ):
        ...
        if not hasattr(ants_platform_client, "start_span"):
    ...
    def test_metrics_and_timing(self, ants_platform_client, memory_exporter):
        ...
        # Create a span
        span = ants_platform_client.start_span(name="timing-test-span")
...
class TestMediaHandling(TestOTelBase):
    ...
    def test_masking(self):
        """Test the masking functionality of AntsPlatform."""
        ...
        # Now test using the actual AntsPlatformSpan implementation
        from unittest.mock import MagicMock

        from ants_platform._client.span import AntsPlatformSpan

        # Create a mock AntsPlatform client with the masking function
        mock_client = MagicMock()
        mock_client._mask = mask_sensitive_data

        # Create a concrete AntsPlatformSpan instance
        mock_span = MagicMock()
        span = AntsPlatformSpan(otel_span=mock_span, ants_platform_client=mock_client)
...
class TestOtelIdGeneration(TestOTelBase):
    ...
    @pytest.fixture
    def ants_platform_client(self, monkeypatch):
        """Create a minimal AntsPlatform client for testing ID generation functions."""
        client = AntsPlatform(
            public_key="test-public-key",
            secret_key="test-secret-key",
            host="http://test-host",
    ...
    def test_trace_id_without_seed(self, ants_platform_client, monkeypatch):
    ...
    def test_trace_id_with_seed(self, ants_platform_client):
        ...
        trace_id_repeat = ants_platform_client.create_trace_id(seed=seed)
        assert trace_id == trace_id_repeat

        # Verify a different seed produces a different ID
        different_seed = "different-identifier"
        different_trace_id = ants_platform_client.create_trace_id(seed=different_seed)
        assert trace_id != different_trace_id

    def test_observation_id_without_seed(self, ants_platform_client, monkeypatch):
        ...
        observation_id = ants_platform_client._create_observation_id()
        assert observation_id == "1234567890abcdef"
        assert len(observation_id) == 16  # 8 bytes hex-encoded = 16 characters

    def test_observation_id_with_seed(self, ants_platform_client):
        """Test observation_id generation with seed (should be deterministic)."""
        seed = "test-identifier"
        observation_id = ants_platform_client._create_observation_id(seed=seed)
        ...
        # Verify the same seed produces the same ID
        observation_id_repeat = ants_platform_client._create_observation_id(seed=seed)
        assert observation_id == observation_id_repeat

        # Verify a different seed produces a different ID
        different_seed = "different-identifier"
        different_observation_id = ants_platform_client._create_observation_id(
            seed=different_seed
        )
        assert observation_id != different_observation_id

    def test_id_generation_consistency(self, ants_platform_client):
        ...
        trace_ids = [ants_platform_client.create_trace_id(seed=seed) for _ in range(5)]
        observation_ids = [
            ants_platform_client._create_observation_id(seed=seed) for _ in range(5)
        ]
    ...
    def test_different_seeds_produce_different_ids(self, ants_platform_client):
        """Test that different seeds produce different IDs."""
        seeds = [f"test-seed-{i}" for i in range(10)]

        # Generate IDs with different seeds
        trace_ids = [ants_platform_client.create_trace_id(seed=seed) for seed in seeds]
        observation_ids = [
            ants_platform_client._create_observation_id(seed=seed) for seed in seeds
        ]
    ...
    def test_ants_platform_event_update_immutability(self, ants_platform_client, caplog):
        """Test that AntsPlatformEvent.update() logs a warning and does nothing."""
        import logging

        parent_span = ants_platform_client.start_span(name="parent-span")
        ...
        with caplog.at_level(logging.WARNING, logger="ants_platform._client.span"):
            result = event.update(
                name="updated_name",
                input={"updated": "input"},
            ...
            assert "Attempted to update AntsPlatformEvent observation" in caplog.text
