...
def test_reference_string():
    ...
    assert (
        reference
        == "@@@ants_platformMedia:type=image/jpeg|id=MwoGlsMS6lW8ijWeRyZKfD|source=bytes@@@"
    )


def test_parse_reference_string():
    valid_ref = "@@@ants_platformMedia:type=image/jpeg|id=test-id|source=base64_data_uri@@@"
...
def test_parse_invalid_reference_string():
    ...
    with pytest.raises(ValueError):
        LangfuseMedia.parse_reference_string(
            "@@@ants_platformMedia:type=image/jpeg@@@"
        )  # Missing fields
...
def test_replace_media_reference_string_in_object():
    ...
    # Create AntsPlatform client and trace with media
    ...
    assert re.match(
        r"^@@@ants_platformMedia:type=audio/wav\|id=.+\|source=base64_data_uri@@@$",
        media_ref,
    )

    # Resolve media references back to base64
    resolved_obs = ants_platform.resolve_media_references(
    ...
    span2 = ants_platform.start_span(
