...
from ants_platform.api.resources.commons.types.dataset_status import DatasetStatus
from ants_platform.api.resources.commons.types.observation import Observation
...
def test_create_and_get_dataset():
    ...
    ants_platform.create_dataset(name=name)
    dataset = ants_platform.get_dataset(name)
    assert dataset.name == name

    name = create_uuid()
    ants_platform.create_dataset(
...
def test_create_dataset_item():
    ...
    generation = ants_platform.start_generation(name="test").end()
    ants_platform.flush()

    input = {"input": "Hello World"}
    ants_platform.create_dataset_item(dataset_name=name, input=input)
...
def test_get_all_items():
    ...
    dataset_2 = ants_platform.get_dataset(name, fetch_items_page_size=9)
    assert len(dataset_2.items) == 99

    dataset_3 = ants_platform.get_dataset(name, fetch_items_page_size=2)
...
def test_upsert_and_get_dataset_item():
    ...
    ants_platform.create_dataset(name=name)
    input = {"input": "Hello World"}
    item = ants_platform.create_dataset_item(
...
def test_dataset_run_with_metadata_and_description():
    ...
    ants_platform.create_dataset(name=dataset_name)

    input = json.dumps({"input": "Hello World"})
    ants_platform.create_dataset_item(dataset_name=dataset_name, input=input)

    dataset = ants_platform.get_dataset(dataset_name)
...
def test_get_dataset_runs():
    ...
    ants_platform.flush()
    time.sleep(1)  # Give API time to process
    runs = ants_platform.api.datasets.get_runs(dataset_name)
