...
def openai():
    ...
    from ants_platform.openai import openai as _openai
...
def test_openai_chat_completion_with_ants_platform_prompt(openai):
    generation_name = create_uuid()
    ants_platform = AntsPlatform()
    ...
    prompt_client = ants_platform.get_prompt(name=prompt_name)

    openai.OpenAI().chat.completions.create(
        name=generation_name,
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": "Make me laugh"}],
        ants_platform_prompt=prompt_client,
    )
...
def test_openai_completion_with_ants_platform_prompt(openai):
    generation_name = create_uuid()
    ants_platform = AntsPlatform()
    prompt_name = create_uuid()
...
def test_base_64_image_input(openai):
    ...
    assert (
        f"@@@ants_platformMedia:type={content_type}|id="
...
def test_audio_input_and_output(openai):
    client = openai.OpenAI()
    openai.ants_platform_debug = True
    ...
    assert (
        "@@@ants_platformMedia:type=audio/wav|id="
