...
from ants_platform import AntsPlatform, get_client, observe
from ants_platform._client.environment_variables import ANTS_PLATFORM_PUBLIC_KEY
...
def test_nested_observations():
    ...
    def level_3_function():
        ...
        ants_platform.update_current_generation(version="version-1")
        ants_platform.update_current_trace(session_id=mock_session_id, name=mock_name)
    ...
    def level_2_function():
        level_3_function()
        ants_platform.update_current_span(metadata=mock_metadata)
    ...
    result = level_1_function(
        *mock_args, **mock_kwargs, ants_platform_trace_id=mock_trace_id
    )
    ants_platform.flush()
...
def test_exception_in_wrapped_function():
    ...
    with pytest.raises(ValueError):
        level_1_function(*mock_args, **mock_kwargs, ants_platform_trace_id=mock_trace_id)
...
def test_concurrent_decorator_executions():
    ...
    ants_platform = get_client()
    mock_trace_id_1 = ants_platform.create_trace_id()
    mock_trace_id_2 = ants_platform.create_trace_id()

    @observe(as_type="generation", capture_output=False)
    def level_3_function():
        ants_platform.update_current_generation(metadata=mock_metadata)
        ants_platform.update_current_generation(metadata=mock_deep_metadata)
        ants_platform.update_current_generation(
            metadata=mock_deep_metadata,
            usage_details={"input": 150, "output": 50, "total": 300},
            model="gpt-3.5-turbo",
        )
        ants_platform.update_current_trace(name=mock_name, session_id=mock_session_id)
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        future1 = executor.submit(
            ...
            *mock_args,
            mock_trace_id_1,
            **mock_kwargs,
            ants_platform_trace_id=mock_trace_id_1,
        )
        future2 = executor.submit(
            ...
            *mock_args,
            mock_trace_id_2,
            **mock_kwargs,
            ants_platform_trace_id=mock_trace_id_2,
        )
...
def test_decorators_langchain():
    ...
    def langchain_operations(*args, **kwargs):
        # Get ants_platform callback handler for LangChain
    ...
    def level_3_function(*args, **kwargs):
        ants_platform.update_current_span(metadata=mock_metadata)
        ants_platform.update_current_span(metadata=mock_deep_metadata)
        ants_platform.update_current_trace(session_id=mock_session_id, name=mock_name)
    ...
    level_1_function(topic="socks", ants_platform_trace_id=mock_trace_id)
...
def test_get_current_trace_url():
    ...
    @observe()
    def level_3_function():
        return ants_platform.get_trace_url(trace_id=ants_platform.get_current_trace_id())
...
def test_scoring_observations():
    ...
    @observe(as_type="generation", capture_output=False)
    def level_3_function():
        ants_platform.score_current_span(name="test-observation-score", value=1)
        ants_platform.score_current_trace(name="another-test-trace-score", value="my_value")
    ...
    @observe()
    def level_1_function(*args, **kwargs):
        ants_platform.score_current_trace(name="test-trace-score", value=3)
        ants_platform.update_current_trace(name=mock_name)
        return level_2_function()
...
def test_circular_reference_handling():
    ...
    result = function_with_circular_arg(circular_obj, ants_platform_trace_id=mock_trace_id)
...
def test_disabled_io_capture():
    ...
    def nested(*args, **kwargs):
        ants_platform.update_current_span(
    ...
    result = main("Hello, World!", name="John", ants_platform_trace_id=mock_trace_id)
...
def test_generator_as_return_value():
    ...
    result = main(ants_platform_trace_id=mock_trace_id)
    ants_platform.flush()
...
async def test_async_generator_as_return_value():
    ...
    result = await main_async(ants_platform_trace_id=mock_trace_id)
    ants_platform.flush()
...
async def test_async_nested_openai_chat_stream():
    from ants_platform.openai import AsyncOpenAI
...
def test_nest_list_of_generator_as_function_IO():
    ...
    main(ants_platform_trace_id=mock_trace_id)
    ants_platform.flush()
...
def test_return_dict_for_output():
    ...
    result = function(ants_platform_trace_id=mock_trace_id)
    ants_platform.flush()
...
def test_media():
    ...
    assert (
        "@@@ants_platformMedia:type=application/pdf|id="
...
def test_merge_metadata_and_tags():
    ...
    @observe
    def nested():
        ants_platform.update_current_trace(metadata={"key2": "value2"}, tags=["tag2"])

    @observe
    def main():
        ants_platform.update_current_trace(metadata={"key1": "value1"}, tags=["tag1"])
...
def test_multiproject_context_propagation_basic():
    """Test that nested decorated functions inherit ants_platform_public_key from parent in multi-project setup"""
    client1 = AntsPlatform()  # Reads from environment
    AntsPlatform(public_key="pk-test-project2", secret_key="sk-test-project2")
    ...
    env_public_key = os.environ[ANTS_PLATFORM_PUBLIC_KEY]
    # In multi-project setup, must specify which client to use
    ants_platform = get_client(public_key=env_public_key)
    mock_trace_id = ants_platform.create_trace_id()

    @observe(as_type="generation", capture_output=False)
    def level_3_function():
        # This function should inherit the public key from level_1_function
        # and NOT need ants_platform_public_key parameter
        ants_platform_client = get_client()
        ants_platform_client.update_current_generation(metadata={"level": "3"})
        ants_platform_client.update_current_trace(name=mock_name)
        return "level_3"
    ...
    def level_2_function():
        ...
        level_3_function()
        ants_platform_client = get_client()
        ants_platform_client.update_current_span(metadata={"level": "2"})
        return "level_2"

    @observe()
    def level_1_function(*args, **kwargs):
        # Only this top-level function receives ants_platform_public_key
        level_2_function()
        ants_platform_client = get_client()
        ants_platform_client.update_current_span(metadata={"level": "1"})
        return "level_1"

    result = level_1_function(
        *mock_args,
        **mock_kwargs,
        ants_platform_trace_id=mock_trace_id,
        ants_platform_public_key=env_public_key,  # Only provided to top-level function
    )
...
def test_multiproject_context_propagation_deep_nesting():
    ...
    def level_4_function():
        ants_platform_client = get_client()
        ants_platform_client.update_current_generation(metadata={"level": "4"})
        return "level_4"
    ...
    def level_3_function():
        ...
        ants_platform_client = get_client()
        ants_platform_client.update_current_span(metadata={"level": "3"})
        return result
    ...
    result = level_1_function(
        ants_platform_trace_id=mock_trace_id, ants_platform_public_key=env_public_key
    )
    client1.flush()
...
def test_multiproject_context_propagation_override():
    # Initialize two separate AntsPlatform instances
    client1 = AntsPlatform()  # Reads from environment
    client2 = AntsPlatform(public_key="pk-test-project2", secret_key="sk-test-project2")
    ...
    def level_3_function():
        ...
        ants_platform_client = get_client(public_key=override_public_key)
        ants_platform_client.update_current_generation(metadata={"used_override": "true"})
        return "level_3"

    @observe()
    def level_2_function():
        # This function should use the overridden key when calling level_3
        level_3_function(ants_platform_public_key=override_public_key)
        ants_platform_client = get_client(public_key=primary_public_key)
        ants_platform_client.update_current_span(metadata={"level": "2"})
        return "level_2"
    ...
    result = level_1_function(
        ants_platform_trace_id=mock_trace_id, ants_platform_public_key=primary_public_key
    )
    client1.flush()
    client2.flush()
...
def test_multiproject_context_propagation_no_public_key():
    ...
    # No ants_platform_public_key provided - should use default client
    result = level_1_function(ants_platform_trace_id=mock_trace_id)
    client1.flush()
...
async def test_multiproject_async_context_propagation_basic():
    """Test that nested async decorated functions inherit ants_platform_public_key from parent in multi-project setup"""
    client1 = AntsPlatform()  # Reads from environment
    AntsPlatform(public_key="pk-test-project2", secret_key="sk-test-project2")
    ...
    async def async_level_3_function():
        ...
        ants_platform_client = get_client()
        ants_platform_client.update_current_generation(
    ...
    async def async_level_2_function():
        ...
        ants_platform_client = get_client()
        ants_platform_client.update_current_span(metadata={"level": "2", "async": True})
        return result
    ...
    async def async_level_1_function(*args, **kwargs):
        ...
        ants_platform_client = get_client()
        ants_platform_client.update_current_span(metadata={"level": "1", "async": True})
        return result
...
async def test_multiproject_mixed_sync_async_context_propagation():
    ...
    async def async_level_3_function():
        ...
        ants_platform_client = get_client()
        ants_platform_client.update_current_span(metadata={"level": "3", "type": "async"})
        return result
    ...
    async def async_level_2_function():
        ...
        result = await async_level_3_function()
        ants_platform_client = get_client()
        ants_platform_client.update_current_span(metadata={"level": "2", "type": "async"})
        return result
    ...
    async def async_level_1_function(*args, **kwargs):
        ...
        ants_platform_client.update_current_trace(name=mock_name)
        result = await async_level_2_function()
        ants_platform_client.update_current_span(metadata={"level": "1", "type": "async"})
        return result
...
async def test_multiproject_concurrent_async_context_isolation():
    ...
    trace_id_1 = ants_platform.create_trace_id()
    trace_id_2 = ants_platform.create_trace_id()
    ...
    async def async_level_2_function(task_id):
        ...
        ants_platform_client = get_client()
        ants_platform_client.update_current_span(metadata={"task_id": task_id, "level": "2"})
        return result

    @observe()
    async def async_level_1_function(task_id, *args, **kwargs):
        ants_platform_client = get_client()
        ants_platform_client.update_current_trace(name=f"{mock_name}_task_{task_id}")
        result = await async_level_2_function(task_id)
        ants_platform_client.update_current_span(metadata={"task_id": task_id, "level": "1"})
        return result

    # Run two concurrent async tasks with the same public key but different trace contexts
    task1 = async_level_1_function(
        "1", ants_platform_trace_id=trace_id_1, ants_platform_public_key=public_key_1
    )
    task2 = async_level_1_function(
        "2", ants_platform_trace_id=trace_id_2, ants_platform_public_key=public_key_2
    )
...
async def test_multiproject_async_generator_context_propagation():
    ...
    async def async_consumer_function():
        ...
        ants_platform_client.update_current_span(
...
async def test_multiproject_async_context_exception_handling():
    ...
    async def async_failing_function():
        ...
        await asyncio.sleep(0.01)
        ants_platform_client = get_client()
        ants_platform_client.update_current_generation(metadata={"will_fail": True})
        ants_platform_client.update_current_trace(name=mock_name)
    ...
    async def async_caller_function():
        try:
            ...
            ants_platform_client = get_client()
            ants_platform_client.update_current_span(metadata={"caught_exception": True})
    ...
    async def async_root_function(*args, **kwargs):
        ...
        ants_platform_client = get_client()
        ants_platform_client.update_current_span(metadata={"root": True})
        return result
