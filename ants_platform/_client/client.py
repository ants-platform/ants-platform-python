"""AntsPlatform OpenTelemetry integration module.

This module implements AntsPlatform's core observability functionality on top of the OpenTelemetry (OTel) standard.
"""
...
from ants_platform._client.datasets import DatasetClient, DatasetItemClient
from ants_platform._client.environment_variables import (
    ANTS_PLATFORM_DEBUG,
    ANTS_PLATFORM_HOST,
    ANTS_PLATFORM_PUBLIC_KEY,
    ANTS_PLATFORM_SAMPLE_RATE,
    ANTS_PLATFORM_SECRET_KEY,
    ANTS_PLATFORM_TIMEOUT,
    ANTS_PLATFORM_TRACING_ENABLED,
...
from ants_platform._utils import _get_timestamp
from ants_platform._utils.parse_error import handle_fern_exception
from ants_platform._utils.prompt_cache import PromptCache
from ants_platform.api.resources.commons.errors.error import Error
from ants_platform.api.resources.ingestion.types.score_body import ScoreBody
from ants_platform.api.resources.prompts.types import (
...
from ants_platform.model import (
    ChatMessageDict,
...
from ants_platform.types import MaskFunction, ScoreDataType, SpanLevel, TraceContext


class AntsPlatform:
    """Main client for AntsPlatform tracing and platform features.

    This class provides an interface for creating and managing traces, spans,
    and generations in AntsPlatform as well as interacting with the AntsPlatform API.
    ...
    Attributes:
        api: Synchronous API client for AntsPlatform backend communication
        async_api: Asynchronous API client for AntsPlatform backend communication
        ants_platform_tracer: Internal LangfuseTracer instance managing OpenTelemetry components
    ...
    Parameters:
        ...
        tracing_enabled (Optional[bool]): Enable or disable tracing. Defaults to True. Can also be set via ANTS_PLATFORM_TRACING_ENABLED environment variable.
        flush_at (Optional[int]): Number of spans to batch before sending to the API. Defaults to 512. Can also be set via ANTS_PLATFORM_FLUSH_AT environment variable.
        flush_interval (Optional[float]): Time in seconds between batch flushes. Defaults to 5 seconds. Can also be set via ANTS_PLATFORM_FLUSH_INTERVAL environment variable.
        environment (Optional[str]): Environment name for tracing. Default is 'default'. Can also be set via ANTS_PLATFORM_TRACING_ENVIRONMENT environment variable. Can be any lowercase alphanumeric string with hyphens and underscores that does not start with 'ants-platform'.
        release (Optional[str]): Release version/hash of your application. Used for grouping analytics by release.
        media_upload_thread_count (Optional[int]): Number of background threads for handling media uploads. Defaults to 1. Can also be set via ANTS_PLATFORM_MEDIA_UPLOAD_THREAD_COUNT environment variable.
        sample_rate (Optional[float]): Sampling rate for traces (0.0 to 1.0). Defaults to 1.0 (100% of traces are sampled). Can also be set via ANTS_PLATFORM_SAMPLE_RATE environment variable.
        mask (Optional[MaskFunction]): Function to mask sensitive data in traces before sending to the API.
        blocked_instrumentation_scopes (Optional[List[str]]): List of instrumentation scope names to block from being exported to AntsPlatform. Spans from these scopes will be filtered out before being sent to the API. Useful for filtering out spans from specific libraries or frameworks. For exported spans, you can see the instrumentation scope name in the span metadata in AntsPlatform (`metadata.scope.name`)
        ...
        tracer_provider(Optional[TracerProvider]): OpenTelemetry TracerProvider to use for AntsPlatform. This can be useful to set to have disconnected tracing between AntsPlatform and other OpenTelemetry-span emitting libraries. Note: To track active spans, the context is still shared between TracerProviders. This may lead to broken trace trees.

    Example:
        ```python
        from ants_platform.otel import AntsPlatform

        # Initialize the client (reads from env vars if not provided)
        ants_platform = AntsPlatform(
            public_key="your-public-key",
            secret_key="your-secret-key",
            host="https://cloud.ants_platform.com",  # Optional, default shown
        )
    ...
    def __init__(
        ...
        *,
        ...
        sample_rate = sample_rate or float(os.environ.get(ANTS_PLATFORM_SAMPLE_RATE, 1.0))
        if not 0.0 <= sample_rate <= 1.0:
            raise ValueError(
                f"Sample rate must be between 0.0 and 1.0, got {sample_rate}"
            )

        timeout = timeout or int(os.environ.get(ANTS_PLATFORM_TIMEOUT, 5))

        self._tracing_enabled = (
            tracing_enabled
            and os.environ.get(ANTS_PLATFORM_TRACING_ENABLED, "true").lower() != "false"
        )
        if not self._tracing_enabled:
            ants_platform_logger.info(
                "Configuration: AntsPlatform tracing is explicitly disabled. No data will be sent to the AntsPlatform API."
            )

        debug = (
            debug if debug else (os.getenv(ANTS_PLATFORM_DEBUG, "false").lower() == "true")
        )
        if debug:
            logging.basicConfig(
                format="%(asctime)s - %(name)s - %(levelname)s - %(message)s"
            )
            ants_platform_logger.setLevel(logging.DEBUG)

        public_key = public_key or os.environ.get(ANTS_PLATFORM_PUBLIC_KEY)
        if public_key is None:
            ants_platform_logger.warning(
                "Authentication error: AntsPlatform client initialized without public_key. Client will be disabled. "
                "Provide a public_key parameter or set ANTS_PLATFORM_PUBLIC_KEY environment variable. "
            )
            self._otel_tracer = otel_trace_api.NoOpTracer()
            return

        secret_key = secret_key or os.environ.get(ANTS_PLATFORM_SECRET_KEY)
        if secret_key is None:
            ants_platform_logger.warning(
                "Authentication error: AntsPlatform client initialized without secret_key. Client will be disabled. "
                "Provide a secret_key parameter or set ANTS_PLATFORM_SECRET_KEY environment variable. "
            )
            self._otel_tracer = otel_trace_api.NoOpTracer()
            return
    ...
    def start_span(
        ...
        *,
        ...
        Returns:
            A AntsPlatformSpan object that must be ended with .end() when the operation completes
    ...
    def start_as_current_span(
        ...
        *,
        ...
        Returns:
            A context manager that yields a AntsPlatformSpan
        ...
        if trace_context:
            ...
            if trace_id:
                ...
                with otel_trace_api.use_span(
                    ...
                    otel_span.set_attribute(AntsPlatformOtelSpanAttributes.AS_ROOT, True)
    ...
    def _create_observation_from_otel_span(
        ...
        *,
        ...
        if as_type in get_observation_types_list(ObservationTypeGenerationLike):
            ...
            # issue is that AntsPlatformEvent could be returned and that classes have diff. args
            return observation_class(  # type: ignore[return-value,call-arg]
                otel_span=otel_span,
                ants_platform_client=self,
            ...
            # span._otel_span.set_attribute("ants_platform.observation.type", as_type)
            # return span
    ...
    def start_generation(
        ...
        *,
        ...
        Returns:
            A AntsPlatformGeneration object that must be ended with .end() when complete
    ...
    def start_as_current_generation(
        ...
        *,
        ...
        Returns:
            A context manager that yields a AntsPlatformGeneration

        Example:
            ```python
            with ants_platform.start_as_current_generation(
                name="answer-generation",
                model="gpt-4",
                input={"prompt": "Explain quantum computing"}
            ) as generation:
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        Example:
            ```python
            # Create a span
            with ants_platform.start_as_current_observation(name="process-query", as_type="span") as span:
                # Do work
                result = process_data()
                span.update(output=result)
            ...
            with ants_platform.start_as_current_observation(name="web-search", as_type="tool") as tool:
                # Do tool work
                results = search_web(query)
                tool.update(output=results)

            # Create a generation observation
            with ants_platform.start_as_current_observation(
                name="answer-generation",
                as_type="generation",
                model="gpt-4"
            ) as generation:
    ...
    def _get_span_class(
        ...
        Type[AntsPlatformAgent],
        Type[AntsPlatformTool],
        Type[AntsPlatformChain],
        Type[AntsPlatformRetriever],
        Type[AntsPlatformEvaluator],
        Type[AntsPlatformEmbedding],
        Type[AntsPlatformGuardrail],
        Type[AntsPlatformGeneration],
        Type[AntsPlatformEvent],
        Type[AntsPlatformSpan],
    ]:
        """Get the appropriate span class based on as_type."""
        normalized_type = as_type.lower()

        if normalized_type == "agent":
            return AntsPlatformAgent
        elif normalized_type == "tool":
            return AntsPlatformTool
        elif normalized_type == "chain":
            return AntsPlatformChain
        elif normalized_type == "retriever":
            return AntsPlatformRetriever
        elif normalized_type == "evaluator":
            return AntsPlatformEvaluator
        elif normalized_type == "embedding":
            return AntsPlatformEmbedding
        elif normalized_type == "guardrail":
            return AntsPlatformGuardrail
        elif normalized_type == "generation":
            return AntsPlatformGeneration
        elif normalized_type == "event":
            return AntsPlatformEvent
        elif normalized_type == "span":
            return AntsPlatformSpan
        else:
            return AntsPlatformSpan
        ...
        *,
        ...
        with otel_trace_api.use_span(parent_span):
            with self._start_as_current_otel_span_with_processed_media(
                ...
                cost_details=cost_details,
                prompt=prompt,
            ) as ants_platform_span:
                if remote_parent_span is not None:
                    ants_platform_span._otel_span.set_attribute(
                        AntsPlatformOtelSpanAttributes.AS_ROOT, True
                    )

                yield ants_platform_span
    ...
    def update_current_generation(
        ...
        *,
        ...
        Example:
            ```python
            with ants_platform.start_as_current_generation(name="answer-query") as generation:
                # Initial setup and API call
                response = llm.generate(...)

                # Update with results that weren't available at creation time
                ants_platform.update_current_generation(
                    output=response.text,
                    usage_details={
                        "prompt_tokens": response.usage.prompt_tokens,
                        "completion_tokens": response.usage.completion_tokens
        ...
        if current_otel_span is not None:
            generation = AntsPlatformGeneration(
                otel_span=current_otel_span, ants_platform_client=self
            )
    ...
    def update_current_span(
        ...
        *,
        ...
        Example:
            ```python
            with ants_platform.start_as_current_span(name="process-data") as span:
                # Initial processing
                result = process_first_part()

                # Update with intermediate results
                ants_platform.update_current_span(metadata={"intermediate_result": result})

                # Continue processing
                final_result = process_second_part(result)

                # Final update
                ants_platform.update_current_span(output=final_result)
            ```
        """
        if not self._tracing_enabled:
            ants_platform_logger.debug(
    ...
    def update_current_trace(
        ...
        *,
        ...
        This method updates the AntsPlatform trace that the current span belongs to. It's useful for
        adding trace-level metadata like user ID, session ID, or tags that apply to
        the entire AntsPlatform trace rather than just a single observation.

        Args:
            name: Updated name for the AntsPlatform trace
            ...
            tags: List of tags to categorize the AntsPlatform trace
            public: Whether the AntsPlatform trace should be publicly accessible
        ...
        Example:
            ...
            with ants_platform.start_as_current_span(name="handle-request") as span:
                ...
                ants_platform.update_current_trace(
                    user_id=user.id,
        ...
        if current_otel_span is not None:
            existing_observation_type = current_otel_span.attributes.get(  # type: ignore[attr-defined]
                AntsPlatformOtelSpanAttributes.OBSERVATION_TYPE, "span"
            )
    ...
    def create_event(
        ...
        *,
        ...
        The created AntsPlatform Event observation will be the child of the current span in the context.
    ...
    def _create_remote_parent_span(
        ...
        if not self._is_valid_trace_id(trace_id):
            ants_platform_logger.warning(
                f"Passed trace ID '{trace_id}' is not a valid 32 lowercase hex char AntsPlatform trace id. Ignoring trace ID."
            )

        if parent_span_id and not self._is_valid_span_id(parent_span_id):
            ants_platform_logger.warning(
                f"Passed span ID '{parent_span_id}' is not a valid 16 lowercase hex char AntsPlatform span id. Ignoring parent span ID."
            )
    ...
    def _create_observation_id(self, *, seed: Optional[str] = None) -> str:
        """Create a unique observation ID for use with AntsPlatform.

        This method generates a unique observation ID (span ID in OpenTelemetry terms)
        for use with various AntsPlatform APIs. It can either generate a random ID or
        ...
        correlate an external ID with a AntsPlatform observation ID, use the external ID as
        ...
        Example:
            ...
            obs_id = ants_platform.create_observation_id()

            # Generate a deterministic ID based on a seed
            user_obs_id = ants_platform.create_observation_id(seed="user-123-feedback")

            # Correlate an external item ID with a AntsPlatform observation ID
            item_id = "item-789012"
            correlated_obs_id = ants_platform.create_observation_id(seed=item_id)

            # Use the ID with AntsPlatform APIs
            ants_platform.create_score(
                name="relevance",
                value=0.95,
                trace_id=trace_id,
    ...
    def create_trace_id(*, seed: Optional[str] = None) -> str:
        """Create a unique trace ID for use with AntsPlatform.

        This method generates a unique trace ID for use with various AntsPlatform APIs.
        ...
        This method ensures the generated ID meets this requirement. If you need to
        correlate an external ID with a AntsPlatform trace ID, use the external ID as the
        seed to get a valid, deterministic AntsPlatform trace ID.
        ...
        Returns:
            A 32-character lowercase hexadecimal string representing the AntsPlatform trace ID.

        Example:
            ```python
            # Generate a random trace ID
            trace_id = ants_platform.create_trace_id()

            # Generate a deterministic ID based on a seed
            session_trace_id = ants_platform.create_trace_id(seed="session-456")

            # Correlate an external ID with a AntsPlatform trace ID
            external_id = "external-system-123456"
            correlated_trace_id = ants_platform.create_trace_id(seed=external_id)
        ...
        if not seed:
            ...
            return AntsPlatform._format_otel_trace_id(trace_id_int)
    ...
    def create_score(
        ...
        *,
        ...
        This method creates a score for evaluating a AntsPlatform trace or observation. Scores can be
        ...
        Args:
            ...
            session_id: ID of the AntsPlatform session to associate the score with
            dataset_run_id: ID of the AntsPlatform dataset run to associate the score with
            trace_id: ID of the AntsPlatform trace to associate the score with
        ...
        try:
            ...
            ants_platform_logger.exception(
        ...
        *,
        ...
        Example:
            ...
            with ants_platform.start_as_current_generation(name="answer-query") as generation:
                ...
                ants_platform.score_current_span(
                    name="relevance",
                    value=0.85,
                    data_type="NUMERIC",
    ...
    def score_current_trace(
        ...
        *,
        ...
        Example:
            ```python
            with ants_platform.start_as_current_span(name="process-user-request") as span:
                # Process request
                result = process_complete_request()
                span.update(output=result)

                # Score the overall trace
                ants_platform.score_current_trace(
    ...
    def flush(self) -> None:
        """Force flush all pending spans and events to the AntsPlatform API.

        This method manually flushes any pending spans, scores, and other events to the
        AntsPlatform API. It's useful in scenarios where you want to ensure all data is sent
        before proceeding, without waiting for the automatic flush interval.

        Example:
            ```python
            # Record some spans and scores
            with ants_platform.start_as_current_span(name="operation") as span:
                # Do work...
                pass

            # Ensure all data is sent to AntsPlatform before proceeding
            ants_platform.flush()
    ...
    def shutdown(self) -> None:
        """Shut down the AntsPlatform client and flush all pending data.

        This method cleanly shuts down the AntsPlatform client, ensuring all pending data
        ...
        Example:
            ```python
            # Initialize AntsPlatform
            ants_platform = AntsPlatform(public_key="...", secret_key="...")

            # Use AntsPlatform throughout your application
            # ...

            # When application is shutting down
            ants_platform.shutdown()
            ```
        """
        if self._resources is not None:
    ...
    def get_current_trace_id(self) -> Optional[str]:
        ...
        Example:
            ...
            with ants_platform.start_as_current_span(name="process-request") as span:
                ...
                trace_id = ants_platform.get_current_trace_id()
    ...
    def get_current_observation_id(self) -> Optional[str]:
        ...
        Example:
            ```python
            with ants_platform.start_as_current_span(name="process-user-query") as span:
                # Get the current observation ID
                observation_id = ants_platform.get_current_observation_id()
    ...
    def get_trace_url(self, *, trace_id: Optional[str] = None) -> Optional[str]:
        """Get the URL to view a trace in the AntsPlatform UI.

        This method generates a URL that links directly to a trace in the AntsPlatform UI.
        ...
        Returns:
            A URL string pointing to the trace in the AntsPlatform UI,
        ...
        Example:
            ...
            with ants_platform.start_as_current_span(name="process-request") as span:
                trace_url = ants_platform.get_trace_url()
                log.info(f"Processing trace: {trace_url}")

            # Get URL for a specific trace
            specific_trace_url = ants_platform.get_trace_url(trace_id="1234567890abcdef1234567890abcdef")
    ...
    def get_dataset(
        ...
        """
        try:
            ants_platform_logger.debug(f"Getting datasets {name}")
            ...
            items = [DatasetItemClient(i, ants_platform=self) for i in dataset_items]
    ...
    def create_dataset(
        ...
        *,
        ...
        """Create a dataset with the given name on AntsPlatform.
        ...
        Returns:
            Dataset: The created dataset as returned by the AntsPlatform API.
        """
        try:
            body = CreateDatasetRequest(
                name=name, description=description, metadata=metadata
            )
            ants_platform_logger.debug(f"Creating datasets {body}")
    ...
    def create_dataset_item(
        ...
        *,
        ...
        Returns:
            DatasetItem: The created dataset item as returned by the AntsPlatform API.
        ...
        Example:
            ...
            # Uploading items to the AntsPlatform dataset named "capital_cities"
            ants_platform.create_dataset_item(
        ...
        try:
            ...
            ants_platform_logger.debug(f"Creating dataset item {body}")
    ...
    def resolve_media_references(
        ...
        *,
        ...
        in the format "@@@ants_platformMedia:...@@@". When found, it (synchronously) fetches the actual media content using
        the provided AntsPlatform client and replaces the reference string with a base64 data URI.
        ...
        Example:
            obj = {
                "image": "@@@ants_platformMedia:type=image/jpeg|id=123|source=bytes@@@",
                "nested": {
                    "pdf": "@@@ants_platformMedia:type=application/pdf|id=456|source=bytes@@@"
                }
            }

            result = await LangfuseMedia.resolve_media_references(obj, ants_platform_client)
    ...
    def get_prompt(
        ...
        *,
        ...
        Keyword Args:
            ...
            fallback: Union[Optional[List[ChatMessageDict]], Optional[str]]: The prompt string to return if fetching the prompt fails. Important on the first call where no cached prompt is available. Follows AntsPlatform prompt formatting with double curly braces for variables. Defaults to None.
        ...
        ants_platform_logger.debug(f"Getting prompt '{cache_key}'")
        ...
        if cached_prompt.is_expired():
            ants_platform_logger.debug(f"Stale prompt '{cache_key}' found in cache.")
            try:
                # refresh prompt in background thread, refresh_prompt deduplicates tasks
                ants_platform_logger.debug(f"Refreshing prompt '{cache_key}' in background.")
    ...
    def _fetch_prompt_and_update_cache(
        ...
        *,
        ...
        cache_key = PromptCache.generate_cache_key(name, version=version, label=label)
        ants_platform_logger.debug(f"Fetching prompt '{cache_key}' from server...")
    ...
    def create_prompt(
        ...
        *,
        ...
        """Create a new prompt in AntsPlatform.
        ...
        """
        try:
            ants_platform_logger.debug(f"Creating prompt {name=}, {labels=}")
    ...
    def update_prompt(
        ...
        *,
        ...
        """Update an existing prompt version in AntsPlatform. The AntsPlatform SDK prompt cache is invalidated for all prompts witht he specified name.

        Args:
            name (str): The name of the prompt to update.
            version (int): The version number of the prompt to update.
            new_labels (List[str], optional): New labels to assign to the prompt version. Labels are unique across versions. The "latest" label is reserved and managed by AntsPlatform. Defaults to [].

        Returns:
            Prompt: The updated prompt from the AntsPlatform API.
