...
from ants_platform.langchain.utils import _extract_model_name
...
try:
    ...
    raise ModuleNotFoundError(
        "Please install langchain to use the AntsPlatform langchain integration: 'pip install langchain'"
    )
...
class LangchainCallbackHandler(LangchainBaseCallbackHandler):
    def __init__(
        ...
        Args:
            public_key: Optional AntsPlatform public key. If not provided, will use the default client configuration.
            update_trace: Whether to update the AntsPlatform trace with the chains input / output / metadata / name. Defaults to False.
        """
    ...
    def on_llm_new_token(
        ...
        **kwargs: Any,
        ...
        if (
            run_id in self.runs
            and isinstance(self.runs[run_id], AntsPlatformGeneration)
            and run_id not in self.updated_completion_start_time_memo
        ):
            current_generation = cast(AntsPlatformGeneration, self.runs[run_id])
    ...
    def _get_observation_type_from_serialized(
        ...
        """Determine AntsPlatform observation type from LangChain component.
        ...
        Returns:
            The appropriate AntsPlatform observation type string
        """
    ...
    def _parse_ants_platform_trace_attributes_from_metadata(
        self,
        ...
        if "ants_platform_session_id" in metadata and isinstance(
            metadata["ants_platform_session_id"], str
        ):
            attributes["session_id"] = metadata["ants_platform_session_id"]

        if "ants_platform_user_id" in metadata and isinstance(
            metadata["ants_platform_user_id"], str
        ):
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            self._register_ants_platform_prompt(
    ...
    def _register_ants_platform_prompt(
        self,
        *,
        run_id: UUID,
        ...
        """We need to register any passed AntsPlatform prompt to the parent_run_id so that we can link following generations with that prompt.
        ...
        ants_platform_prompt = metadata and metadata.get("ants_platform_prompt", None)

        if ants_platform_prompt:
            self.prompt_to_parent_run_map[parent_run_id] = ants_platform_prompt
    ...
    def _deregister_ants_platform_prompt(self, run_id: Optional[UUID]) -> None:
        ...
        try:
            ...
            if agent_run is not None:
                agent_run._otel_span.set_attribute(
                    AntsPlatformOtelSpanAttributes.OBSERVATION_TYPE, "agent"
                )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if span is not None:
                ...
                self._deregister_ants_platform_prompt(run_id)
        ...
        try:
            ...
            content = {
                ...
                "metadata": self.__join_tags_and_metadata(
                    ...
                    keep_ants_platform_trace_attributes=True
                    if parent_run_id is None
                    else False,
                ),
                "model": model_name,
    ...
    def _log_model_parse_warning(self) -> None:
        if not hasattr(self, "_model_parse_warning_logged"):
            ants_platform_logger.warning(
                "AntsPlatform was not able to parse the LLM model. The LLM call will be recorded without model name. Please create an issue: https://github.com/ants_platform/ants_platform/issues/new/choose"
            )
    ...
    def __join_tags_and_metadata(
        ...
        tags: Optional[List[str]] = None,
        metadata: Optional[Dict[str, Any]] = None,
        keep_ants_platform_trace_attributes: bool = False,
...
def _parse_usage_model(usage: typing.Union[pydantic.BaseModel, dict]) -> Any:
    ...
    for model_key, ants_platform_key in conversion_list:
...
def _strip_ants_platform_keys_from_dict(
    metadata: Optional[Dict[str, Any]], keep_ants_platform_trace_attributes: bool
) -> Any:
    if metadata is None or not isinstance(metadata, dict):
        return metadata

    ants_platform_metadata_keys = [
        "ants_platform_prompt",
    ]

    ants_platform_trace_attribute_keys = [
        "ants_platform_session_id",
        "ants_platform_user_id",
        "ants_platform_tags",
    ]

    metadata_copy = metadata.copy()

    for key in ants_platform_metadata_keys:
        metadata_copy.pop(key, None)

    if not keep_ants_platform_trace_attributes:
        for key in ants_platform_trace_attribute_keys:
            metadata_copy.pop(key, None)
